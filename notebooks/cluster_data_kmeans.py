...
def collect_data(input_dir, dem_dir, num_workers=None):
    ...
    with multiprocessing.Pool(processes=num_workers) as pool:
        ...
        # Note: pool.imap() automatically unpacks iterable arguments when used in multiprocessing, 
        # but our function collect_data_for_subdir() expects a single tuple as an argument.
        # Use starmap for multiple arguments.

    # Combine results
    elevation_values, temperature_values, humidity_values = zip(*results)
...
def apply_kmeans_clustering(elevation_values, temperature_values, humidity_values, n_clusters=8):
    """Apply KMeans clustering to the data."""
    # Ensure all arrays have the same length
    assert elevation_values.shape == temperature_values.shape == humidity_values.shape, "Data shapes do not match."

    # Stack the values into a single matrix for clustering
    data = np.stack([elevation_values, temperature_values, humidity_values], axis=1)
...
def process_file(args):
    ...
    data_point = np.array([elevation, temperature, humidity]).reshape(1, -1)
...
if __name__ == "__main__":
    ...
    elevation_values, temperature_values, humidity_values = collect_data(input_dir=input_directory, dem_dir=dem_directory)

    # Apply KMeans clustering to the collected data
    print("Applying KMeans clustering...")
    kmeans_model = apply_kmeans_clustering(elevation_values, temperature_values, humidity_values, n_clusters=8)
