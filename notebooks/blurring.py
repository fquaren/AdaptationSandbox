...
def process_file(file_info):
    ...
    try:
        ...
        output_path = os.path.join(
            output_dir, file.replace(".nz", f"_blurred_x{downsampling_factor}.nz")
        )
...
def main():
    ...
    input_root_dir = os.path.join(
        data_path, "1d-PS-RELHUM_2M-T_2M_cropped_gridded_clustered_threshold_12"
    )
    ...
    for cluster_dir in tqdm(cluster_dirs):
        ...
        input_files = glob.glob(
            os.path.join(cluster_path, "*.nz")
        )  # Adjust file extension if needed
        file_list.extend(
            [
                (os.path.basename(f), cluster_dir, downsampling_factor)
                for f in input_files
            ]
        )

    logging.info(
        f"Found {len(file_list)} files across {len(cluster_dirs)} clusters to process."
    )
